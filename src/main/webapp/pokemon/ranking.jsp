      ...
      <table class="table table-hover table-striped mb-0">
        ...
        <tbody>
          <c:forEach var="score" items="${highScores}" varStatus="status">
            <tr
              class="${status.index < 3 ? (status.index == 0 ? 'pokemon-rank-1' : (status.index == 1 ? 'pokemon-rank-2' : 'pokemon-rank-3')) : ''}"
            >
              <td>
                <c:choose>
                  ...
                  <c:when test="${status.index == 1}">
                    <h5>
                      <i class="bi bi-award-fill text-secondary"></i> 🥈 #2
                    </h5>
                  </c:when>
              ...
              <td class="text-center">
                <h4>
                  <span class="badge bg-primary">
                    <fmt:formatNumber
                      value="${score.score / score.tries * 100}"
                      maxFractionDigits="1"
                    />%
                  </span>
                </h4>
