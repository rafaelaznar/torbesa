package net.ausiasmarch.starwars.dao;
...
public class ScoreDao {
    ...
    public ScoreDto get(int userId) throws SQLException {
        ...
        String sql = "SELECT * FROM starwars_score, users WHERE starwars_score.user_id = ? AND users.id = ? ORDER BY timestamp DESC";
    ...
    public int count(int userId) throws SQLException {
        int count = 0;
        String sql = "SELECT COUNT(*) AS score_count FROM starwars_score WHERE user_id = ?";
        PreparedStatement stmt = oConnection.prepareStatement(sql);
        stmt.setInt(1, userId);
        ResultSet rs = stmt.executeQuery();
        if (rs.next()) {
    ...
    public int count() throws SQLException {
        String countSql = "SELECT COUNT(*) AS total FROM starwars_score";
    ...
    public void sanitize() throws SQLException {
        // Si un usuario tiene más de una puntuación, 
        // entonces se borrarán todas las puntuaciones en la base de datos, excepto la última
        String sql = "DELETE s1 FROM starwars_score s1 " +
                "INNER JOIN starwars_score s2 ON s1.user_id = s2.user_id " +
    ...
    public List<ScoreDto> getTop10() throws SQLException {
        ...
        String sql = "SELECT * FROM starwars_score, users ";
        sql += "WHERE starwars_score.user_id = users.id ";
        sql += "ORDER BY starwars_score.score DESC, starwars_score.timestamp DESC LIMIT 10";
    ...
    public List<ScoreDto> getAll() throws SQLException {
        String getAllString = "SELECT * FROM starwars_score ORDER BY user_id ASC";
    ...
    public int insert(ScoreDto oScore) throws SQLException {
        String insertSql = "INSERT INTO starwars_score (user_id, score, tries, timestamp) VALUES (?, ?, ?, NOW())";
    ...
    public int update(ScoreDto oScore) throws SQLException {
        String updateSql = "UPDATE starwars_score SET score = ?, tries = ?, timestamp = NOW() WHERE user_id = ?";
    ...
    public int delete(int id) throws SQLException {
        String deleteSql = "DELETE FROM starwars_score WHERE id = ?";
